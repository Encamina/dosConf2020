...
function UpdateResults(data) {
    ...
    for (var i = 0; i < data.results.length; i++) {
        ...
        var content = result.myOcrText;
        // get path
        if (data.isPathBase64Encoded) {
            path = Base64Decode(result.id) + token;
        }
        else {
        ...
        if (result["metadata_storage_name"] !== undefined) {
            name = result.name;//result.metadata_storage_name.split(".")[0];
        }
        ...
        if (path !== null) {
            ...
            resultsHtml += `<div class="${classList}" );">
                                    <div class="search-result">
                                        <div class="results-header">
                                            <h4>Could not get id for this result.</h4>
                                        </div>
                                    </div>
                                </div>`; 
        }
